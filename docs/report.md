...
*Generated 2026-08-19 03:22 UTC by [sol-sentinel](https://github.com/Arnav23apr/sol-sentinel) - auto-updating, keyless, Python-stdlib-only.*
...
| Slot | 440M |
| Block height | 418M |
| Epoch | 1018 (95.03% complete, ~2.5h remaining) |
| Lifetime transactions | 539.5B |
| Circulating supply | 582.9M SOL |
| Inflation (annual) | 3.69% |
| Median transaction fee | 5,567 lamports (about $0.00043) |
| Transaction fee p90 / p99 | 21,173 / 483,174 lamports |
| Paying no priority fee | 18.40% of 6,307 sampled transactions (compared against the 5,000-lamport single-signature base fee, so multi-signature transactions are not counted here) |
...
|---|---|---|---|
| SPL Token | 31,899 | 9.30% | 1.7 s |
| Pump.fun | 16,258 | 82.60% | 3.3 s |
| Jupiter v6 | 2,682 | 59.50% | 22.3 s |
| Orca Whirlpools | 651 | 52.00% | 91.9 s |
| Raydium AMM v4 | 536 | 11.10% | 111.3 s |

Median failure rate across the sampled programs: **52.00%** (range 9.30% to 82.60%). A median over five programs, not a chain-wide rate, and it varies widely between them.

Unwithdrawn inflation rewards sitting in the top 8 vote accounts: **35.28 SOL**.

## Exchange and large-holder balances

12.16M SOL ($936.50M) across 8 publicly-attributed accounts. Net **22K SOL (0.18%) moved off exchanges** over the last 23.3 h.

| Account | Balance (SOL) | Value | Activity | Failed |
|---|---|---|---|---|
| Binance | 9.75M | $750.65M | 2.9/h | 0 |
| Binance (2) | 1.53M | $117.82M | 676.7/h | 0 |
| Gate.io | 392.92K | $30.25M | 88.2/h | 0 |
| Bybit | 325.52K | $25.06M | 35.4/h | 0 |
| Coinbase | 50.82K | $3.91M | 303.8/h | 0 |
| Coinbase (2) | 46.20K | $3.56M | 362.2/h | 0 |
| Bitget | 38.53K | $2.97M | 94/h | 0 |
| Kraken | 29.55K | $2.28M | 157.6/h | 0 |

*Balances are re-verified on the chain every run and an account is dropped if it no longer holds a meaningful amount, so a stale label cannot become a false claim. Attribution is best-effort from public sources: Sentinel verifies what an account holds, never who controls it.*

## What moves together

Relationships between metrics, rather than each metric on its own. 5 of 117 tested pairs survive, over 433 observations.

*Method: Spearman rank correlation of period-over-period changes, with Benjamini-Hochberg false-discovery control at q=0.05 across all pairs tested. Changes are correlated rather than levels, because two series that both drift upward correlate near +1 whatever the real relationship. Rank correlation is used so a single outlier cannot manufacture a result.*

| Relationship | rho | n | p |
|---|---|---|---|
| DeFi TVL moves with xStocks AUM | +0.227 | 392 | 0.0000 |
| AMM write-lock congestion moves with Program failure rate | +0.177 | 364 | 0.0007 |
| Slot time moves with AMM write-lock congestion | +0.166 | 364 | 0.0015 |
| Non-vote TPS moves with Slot time | +0.162 | 405 | 0.0011 |
| DEX volume moves with App fees | +0.160 | 392 | 0.0015 |
...
|---|---|---|---|---|
| Chain fees (24h) | sampled blocks (RPC): 460.73K USD | DeFiLlama: 649.17K USD | -33.96% | *indicative*: 0.71x, within the order-of-magnitude band |
| SOL price | coingecko: 76.99 USD | Jupiter (on-chain DEX): 76.94 USD | +0.06% | agree |
