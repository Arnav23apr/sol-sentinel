...
*Generated 2026-08-26 00:37 UTC by [sol-sentinel](https://github.com/Arnav23apr/sol-sentinel) - auto-updating, keyless, Python-stdlib-only.*
...
| Slot | 442M |
| Block height | 420M |
| Epoch | 1022 (56.92% complete, ~18.8h remaining) |
| Lifetime transactions | 541.9B |
| Circulating supply | 583.4M SOL |
| Inflation (annual) | 3.68% |
| Median transaction fee | 5,469 lamports (about $0.00053) |
| Transaction fee p90 / p99 | 22,825 / 480,001 lamports |
| Paying no priority fee | 22.20% of 6,466 sampled transactions (compared against the 5,000-lamport single-signature base fee, so multi-signature transactions are not counted here) |
| AMM write-lock congestion (150-slot window) | 24.00% of slots needed a priority fee (max 4.4M µlam/CU) |
...
| Metric | Value |
|---|---|
| SOL price | $96.62 (-3.92% 24h) |
| Market cap | $56.37B (rank #7) |
| 24h volume | $5.43B |
| ATH | $293.31 (-67.06% from ATH) |
...
| App fees (24h, all protocols) | $14.62M |
...
| xStocks tokenized-equity AUM | $430.26M |
...
Top tokenized equities: CRCLX ($66.72M), TSLAX ($64.05M), MSTRX ($49.49M), SPYX ($48.28M), GOOGLX ($31.12M)
...
|---|---|---|---|
| SPL Token | 80,033 (approx.) | 39.00% | 0.4 s |
| Pump.fun | 21,688 | 80.10% | 2.5 s |
| Jupiter v6 | 9,561 | 67.20% | 6.2 s |
| Raydium AMM v4 | 4,393 | 23.20% | 13.5 s |
| Orca Whirlpools | 3,668 | 63.60% | 16 s |

Median failure rate across the sampled programs: **63.60%** (range 23.20% to 80.10%). A median over five programs, not a chain-wide rate, and it varies widely between them.

Unwithdrawn inflation rewards sitting in the top 8 vote accounts: **35.31 SOL**.

## Exchange and large-holder balances

12.69M SOL ($1.23B) across 8 publicly-attributed accounts. Net **181K SOL (1.41%) moved off exchanges** over the last 22.4 h.

| Account | Balance (SOL) | Value | Activity | Failed |
|---|---|---|---|---|
| Binance | 9.41M | $909.07M | 1.2/h | 9 |
| Binance (2) | 2.36M | $228.36M | 1.1K/h | 0 |
| Bybit | 377.27K | $36.45M | 390.9/h | 0 |
| Gate.io | 368.15K | $35.57M | 234.4/h | 0 |
| Bitget | 61.42K | $5.93M | 181.8/h | 0 |
| Coinbase (2) | 38.41K | $3.71M | 925.4/h | 0 |
| Coinbase | 37.21K | $3.60M | 645.2/h | 0 |
| Kraken | 35.02K | $3.38M | 295.8/h | 0 |

*Balances are re-verified on the chain every run and an account is dropped if it no longer holds a meaningful amount, so a stale label cannot become a false claim. Attribution is best-effort from public sources: Sentinel verifies what an account holds, never who controls it.*

## What moves together

Relationships between metrics, rather than each metric on its own. 9 of 117 tested pairs survive, over 685 observations.

*Method: Spearman rank correlation of period-over-period changes, with Benjamini-Hochberg false-discovery control at q=0.05 across all pairs tested. Changes are correlated rather than levels, because two series that both drift upward correlate near +1 whatever the real relationship. Rank correlation is used so a single outlier cannot manufacture a result.*

| Relationship | rho | n | p |
|---|---|---|---|
| DeFi TVL moves with xStocks AUM | +0.214 | 644 | 0.0000 |
| DEX volume moves with App fees | +0.188 | 644 | 0.0000 |
| Non-vote TPS moves with Slot time | +0.185 | 651 | 0.0000 |
| Total TPS moves with Slot time | +0.148 | 651 | 0.0001 |
| Non-vote TPS moves with AMM write-lock congestion | +0.126 | 610 | 0.0018 |
| Total TPS moves with AMM write-lock congestion | +0.123 | 610 | 0.0023 |
| Slot time moves with AMM write-lock congestion | +0.123 | 610 | 0.0023 |
| Non-vote TPS moves with Activity index | +0.119 | 650 | 0.0024 |
| AMM write-lock congestion moves with Program failure rate | +0.119 | 610 | 0.0033 |
...
|---|---|---|---|---|
| Chain fees (24h) | sampled blocks (RPC): 586.12K USD | DeFiLlama: 928.73K USD | -45.23% | *indicative*: 0.63x, within the order-of-magnitude band |
| SOL price | coingecko: 96.62 USD | Jupiter (on-chain DEX): 96.29 USD | +0.34% | agree |
